...
{!isEmailSent && 
    ...
    <h1 className='text-white text-2xl font-semibold text-center mb-4'>Reset Password</h1>
        ...
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          ...
          <input className='bg-transparent outline-none text-white' placeholder='Email id ' value={email} onChange={e => setEmail(e.target.value)} required type="email" />
        </div>
        ...
        <div className='flex justify-between mb-8' onPaste={handlePaste}>
          {
            Array(6).fill(0).map((_,index) => (
              <input 
                ...
                maxLength='1'
                required
                className='w-12 h-12 text-white text-center text-xl font-bold border border-indigo-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 '
                autoFocus={index===0}
...
{isOtpSubmitted && isEmailSent &&
    ...
    <h1 className='text-white text-2xl font-semibold text-center mb-4'>New Password</h1>
        ...
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          ...
          placeholder='password ' value={newPassword} onChange={e => setNewPassword(e.target.value)} required type="password" />
        </div>
        <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-3'>Submit</button>
    </form>
}
